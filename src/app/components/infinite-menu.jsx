...
class InfiniteGridMenu {
  ...
  #initTexture() {
    ...
    // Draw project titles into each atlas cell instead of images
    const drawCell = (i, title) => {
      ...
      // Title text
      const label = (title || '').toString().trim() || 'Project';
      ctx.fillStyle = '#ffffff';
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      let fontSize = 180; // start large and shrink to fit
      const maxWidth = r * 1.6; // within circle
      while (fontSize > 24) {
    ...
    (this.items.length ? this.items : [{ title: 'Project' }]).forEach((item, i) => {
      drawCell(i, item.title);
    });
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      <canvas
        ...
        ref={canvasRef}
        style={{ width: '100%', height: '100%', display: 'block', cursor: isMoving ? 'grabbing' : 'grab' }}
      />
        ...
        <>
          <p
            style={{
              ...
              top: '50%',
              right: '2vw',
              transform: `translateY(-50%) translateX(${isMoving ? '-60%' : '-90%'})`,
              maxWidth: '18ch',
          ...
          >
            <p style={{ userSelect: 'none', position: 'relative', color: '#060010', top: 2, fontSize: 26 }}>
              &#x2197;
            </p>
          </div>
        </>
